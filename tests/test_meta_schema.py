"""
Tests for the composite FAIR dialect meta-schema (schemas/meta/fair-data-schema.json).
"""
...
def test_fair_meta_schema_has_required_fields(fair_meta_schema: dict) -> None:  # type: ignore[type-arg]
...
def test_fair_meta_schema_id_uses_base_uri(fair_meta_schema: dict) -> None:  # type: ignore[type-arg]
    assert fair_meta_schema["$id"] == "https://highvaluedata.net/fair-data-schema" or fair_meta_schema["$id"].startswith("https://highvaluedata.net/fair-data-schema/")


def test_fair_meta_schema_standard_vocabs_are_required(fair_meta_schema: dict) -> None:  # type: ignore[type-arg]
...
def test_fair_meta_schema_fair_annotations_vocab_is_optional(fair_meta_schema: dict) -> None:  # type: ignore[type-arg]
